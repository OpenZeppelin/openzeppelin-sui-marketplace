...
1. **Contract storage -> objects**: your state lives in owned/shared objects, not in a single contract storage map. See `packages/dapp/move/oracle-market/sources/shop.move` (`Shop` with its listings table, `AcceptedCurrency`, `DiscountTemplate`).
...
- **Object ownership types**: Sui supports address-owned, shared, immutable, and object-owned
  objects. This repo uses address-owned capabilities/tickets, shared objects for the Shop and
  currency/template registries, and object-owned dynamic-field children for markers/claims.
...
Sui: objects + dynamic collections
  Shop (shared)
    listings: Table<u64, ItemListing>
    dynamic fields: currency/template markers
```
