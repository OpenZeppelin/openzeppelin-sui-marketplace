...
- **Ownership types in practice**: `ShopOwnerCap` is address-owned, the Shop is shared, listings
  live inside the Shop’s table, and dynamic-field children are used for currency/template markers
  plus per-claimer discount claims. Sui enforces access based on ownership and the object
  references you pass into a PTB.
  Code: `packages/dapp/move/oracle-market/sources/shop.move` (ShopOwnerCap, Shop, markers)
