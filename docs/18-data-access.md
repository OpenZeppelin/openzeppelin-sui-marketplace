...
This chapter explains how to read Sui data for apps: object queries, table entries, events, and indexer-backed APIs.

## 1. Learning goals
1. Understand the data access layers Sui exposes (RPC, gRPC, GraphQL).
2. Learn how objects, table entries, and events map to UI reads.
...
- **Tables + dynamic fields**: listings and accepted currencies are read from `Shop` table entries (`Shop.listings`, `Shop.accepted_currencies`), while discount-template claims use dynamic-field markers under each template.

Code:
1. `packages/domain/core/src/models/shop.ts` (events + shop overview)
2. `packages/domain/core/src/models/item-listing.ts` (listing table entry lookups)
...
## 7. Exercises
1. Run `pnpm script buyer:shop:view` and compare output with `pnpm script buyer:item-listing:list`. Expected outcome: you can trace which calls enumerate `Shop.listings` table entries.
