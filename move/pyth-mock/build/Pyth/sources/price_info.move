...
public entry fun publish_price_feed(
    feed_id: vector<u8>,
    price_value: u64,
...
public entry fun update_price_feed(
    ...
    let identifier = price_feed::get_price_identifier(&price_info_object.price_info.price_feed);
    let price = build_price(price_value, price_is_negative, conf, expo, expo_is_negative, now_secs);
...
public fun new_price_info_object(price_info: PriceInfo, ctx: &mut tx::TxContext): PriceInfoObject {
    PriceInfoObject {
        id: obj::new(ctx),
        price_info,
    }
