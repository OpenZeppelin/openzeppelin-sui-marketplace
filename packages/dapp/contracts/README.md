...
- **Typed coins and receipts:** Payment assets are `Coin<C>` resources with no approvals; receipts are `ShopItem<T>` whose type must match the listing to keep downstream logic strongly typed. These receipts can be exchanged in a separate fulfillment or on-chain redemption flow for the actual item type. Because shops are permissionless and different shops or listings may reuse the same `T`, redemption logic that must scope to a specific shop or listing should verify provenance through the public `ShopItem<T>` accessors (`shop_id`, `item_listing_id`, `item_type`, `name`, `acquired_at`, `id`) rather than trusting `T` alone.
