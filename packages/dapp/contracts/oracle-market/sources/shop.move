...
// - Shared objects (Shop, AcceptedCurrency, DiscountTemplate): shared objects are globally
//   addressable. Anyone can include them as inputs and read them, and any transaction that mutates
//   them goes through consensus. What "can mutate" really means is "can submit a tx that tries"
//   -- the module still enforces its own authorization checks. Listings are stored inside the
//   shared Shop via a dynamic Table, so listing edits mutate the Shop instead of separate listing
//   objects. These are created with object::new and shared via transfer::share_object.
...
// - Dynamic collections (Table + markers + per-claimer claims): listings live in a Table stored on
//   the Shop, while accepted currencies/templates keep marker entries and discounts track per-claimer
//   claims via dynamic fields. Lookups by key are cheap, but dynamic fields are not enumerable
//   on-chain, so discovery still relies on indexing/off-chain queries. This keeps parent objects
//   small and limits contention to the touched child. Docs: docs/08-listings-receipts.md,
//   docs/10-discounts-tickets.md
...
/// Shared shop that stores listings in a dynamic table plus accepted currencies and discount templates via
/// dynamic fields.
...
/// - Shared object composition: the shop is shared, accepted currencies/templates are shared
///   siblings indexed by lightweight markers, and listings live inside the shop's Table. Listing
///   edits now lock the Shop, trading per-listing objects for a dynamic collection structure.
...
/// - Listings live inside the Shop's Table, so admin flows mutate the Shop to update listings.
///   This demonstrates dynamic collections and keeps listing state compact without standalone
///   listing objects.
...
entry fun update_item_listing_stock(
    ...
    let item_listing = shop.borrow_listing_mut(listing_id, shop_address);
...
///
/// This removes the listing entry from the Shop table. Historical discovery should rely on events.
...
/// - The `Shop` holds the listings table, so checkout mutates the Shop entry for the listing while
///   accepted currency and templates remain separate shared objects.
...
entry fun buy_item<TItem: store, TCoin>(
    ...
    let (owed_coin_opt, change_coin, minted_item) = {
        let item_listing = shop.borrow_listing_mut(listing_id, shop_address);
...
fun process_purchase_core<TItem: store, TCoin>(
    ...
    assert_listing_currency_match!(shop_address, item_listing, accepted_currency);
...
macro fun assert_listing_currency_match(
    ...
    let shop_address = $shop_address;
    let item_listing = $item_listing;
    let accepted_currency = $accepted_currency;
    assert!(item_listing.shop_address == shop_address, EListingShopMismatch);
...
public fun discount_template_values(
    shop: &Shop,
    template: &DiscountTemplate,
): (address, Option<u64>, DiscountRule, u64, Option<u64>, Option<u64>, u64, u64, bool) {
...
#[test_only]
public fun test_minting_completed_listing(event: &MintingCompletedEvent): u64 {
    event.listing_id
}
...
#[test_only]
public fun test_discount_redeemed_listing_id(event: &DiscountRedeemedEvent): u64 {
    event.listing_id
}
...
): (address, address, Option<u64>, address) {
