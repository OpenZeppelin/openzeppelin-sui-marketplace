...
fun remove_currency_if_exists<TCoin>(
    shop: &mut shop::Shop,
    owner_cap: &shop::ShopOwnerCap,
) {
...
fun owner_cap_ids_transferred_to_sender(
    ...
    while (index < transferred_ids.length()) {
        ...
        if (transferred_to_account[&transferred_id] == sender &&
...
fun create_shop_and_owner_cap_ids_with_tx_effects_for_sender(
    ...
    let (created_ids, shared_ids, transferred_to_account) =
...
fun create_default_shop_and_owner_cap_ids_for_sender(
    scn: &mut test_scenario::Scenario,
): (ID, ID) {
...
fun create_two_shops_and_owner_cap_pairs_with_events_for_sender(
    ...
    let (created_ids, shared_ids, transferred_to_account) =
        next_sender_tx_created_shared_ids_and_transfers(scn);
    let (first_shop_id, first_owner_cap_id, second_shop_id, second_owner_cap_id) =
...
fun two_shop_and_owner_cap_pairs_from_tx_ids(
    ...
    if (first_owner_cap_shop_id == first_shop_id && second_owner_cap_shop_id == second_shop_id) {
        (first_shop_id, first_owner_cap_id, second_shop_id, second_owner_cap_id)
    } else if (first_owner_cap_shop_id == second_shop_id &&
...
fun assert_listing_scoped_percent_template(
    ...
    let (rule_kind, rule_value) = shop::test_discount_rule_values(discount_rule);
...
fun assert_single_listing_added_event(shop: &shop::Shop, listing_id: ID) {
    assert_emitted_once!(shop::new_item_listing_added_event(shop::shop_id(shop), listing_id));
}

fun assert_single_discount_template_created_event(shop: &shop::Shop, template_id: ID) {
    assert_emitted_once!(shop::new_discount_template_created_event(
        shop::shop_id(shop),
        template_id,
    ));
}
...
fun create_shop_emits_event_and_records_ids() {
    ...
    assert_eq!(
        emitted_event,
        shop::new_shop_created_event(shop_id, owner_cap_id),
    );
    assert_eq!(shared_ids.length(), 1);
    assert_eq!(shared_ids[0], shop_id);
...
fun create_shop_emits_unique_shop_and_cap_ids() {
    ...
    assert!(
        first_owner_cap_id != second_owner_cap_id,
    );
...
fun update_shop_owner_rotates_payout_and_emits_event() {
    ...
    assert_emitted_once!(shop::new_shop_owner_updated_event(
        shop::shop_id(&shop),
        cap_id,
    ));
...
fun update_shop_owner_emits_event_even_when_unchanged() {
    ...
    assert_emitted_once!(shop::new_shop_owner_updated_event(shop_id, cap_id));
...
fun disable_shop_sets_flag_and_emits_event() {
    ...
    let shop_id = shop::shop_id(&shop);
    let cap_id = shop::shop_owner_cap_id(&owner_cap);
    assert_emitted_once!(shop::new_shop_disabled_event(shop_id, cap_id));
...
fun update_shop_owner_rejects_foreign_cap() {
    ...
    let (mut shop, owner_cap) = shop::test_setup_shop(TEST_OWNER, &mut ctx);
    let (other_shop, other_cap) = shop::test_setup_shop(
        OTHER_OWNER,
        &mut ctx,
    );
...
fun quote_rejects_attestation_lag_above_currency_cap() {
    ...
    let shared_shop = take_shared_shop(&scn, shop_id);
    let mut clock_obj = create_test_clock_at(test_scenario::ctx(&mut scn), (attestation_time + 1) * 1000);
...
fun quote_rejects_price_timestamp_older_than_max_age() {
    ...
    let shared_shop = take_shared_shop(&scn, shop_id);
    let mut clock_obj = create_test_clock_at(test_scenario::ctx(&mut scn), 200_000);
...
fun remove_accepted_currency_removes_state_and_emits_event() {
    ...
    let _first_currency_id = tx_context::last_created_object_id(test_scenario::ctx(&mut scn)).to_id();
...
fun remove_accepted_currency_rejects_missing_id() {
    ...
    let _ = test_scenario::next_tx(&mut scn, OTHER_OWNER);
    let (other_shop_id, other_owner_cap_id) = create_default_shop_and_owner_cap_ids_for_sender(&mut scn);
...
fun quote_view_matches_internal_math() {
    ...
    let mut clock_obj = create_test_clock_at(test_scenario::ctx(&mut scn), 1);
...
fun add_item_listing_stores_metadata() {
    ...
    let spotlight_template_id = shop::listing_values_spotlight_discount_template_id(&listing_values);
    ...
    assert_emitted_once!(shop::new_item_listing_added_event(
        shop::shop_id(&shop),
        listing_id,
    ));
...
fun add_item_listing_with_discount_template_creates_listing_and_pinned_template() {
    ...
    let (listing_id, template_id) = shop::add_item_listing_with_discount_template<
        TestItem,
    >(
        &mut shop,
        &owner_cap,
    ...
    assert_single_listing_added_event(&shop, listing_id);
    assert_single_discount_template_created_event(&shop, template_id);
...
fun update_item_listing_stock_handles_multiple_updates_and_events() {
    ...
    let event_count_before = event::events_by_type<shop::ItemListingStockUpdatedEvent>().length();
    shop::update_item_listing_stock(
        &mut shop,
        &owner_cap,
        listing_id,
    ...
    assert_last_emitted!(event_count_before, expected_stock_updated_event);
    let event_count_before = event::events_by_type<shop::ItemListingStockUpdatedEvent>().length();
    shop::update_item_listing_stock(
        &mut shop,
        &owner_cap,
    ...
    let expected_event = shop::new_item_listing_stock_updated_event(shop::shop_id(&shop), listing_id);
...
fun create_discount_template_persists_fields_and_emits_event() {
    ...
    let (rule_kind, rule_value) = shop::test_discount_rule_values(rule);
...
fun update_discount_template_rejects_after_expiry() {
    ...
    let mut clock_obj = create_test_clock_at(&mut ctx, 200_000);
...
fun toggle_discount_template_updates_active_and_emits_events() {
    ...
    let (rule_kind, rule_value) = shop::test_discount_rule_values(rule);
    let (rule_after_first_kind, rule_after_first_value) = shop::test_discount_rule_values(
        rule_after_first,
    );
    ...
    let (rule_after_second_kind, rule_after_second_value) = shop::test_discount_rule_values(
        rule_after_second,
    );
...
fun claim_discount_ticket_mints_transfers_and_records_claim() {
    ...
    let mut shared_shop = take_shared_shop(&scn, shop_id);
    let mut clock_obj = create_test_clock_at(test_scenario::ctx(&mut scn), 10_000);
    ...
    assert_last_emitted!(
        claim_event_count_before,
        shop::new_discount_claimed_event(shop_id, ticket_id),
    );
...
fun claim_discount_ticket_rejects_before_start_time() {
    ...
    let mut clock_obj = create_test_clock_at(&mut ctx, 5_000);
...
fun claim_discount_ticket_rejects_after_expiry() {
    ...
    let mut clock_obj = create_test_clock_at(&mut ctx, 4_000);
...
fun claim_discount_ticket_rejects_when_maxed() {
    ...
    let mut clock_obj = create_test_clock_at(&mut ctx, 2_000);
    shop::claim_discount_ticket(&mut shop, template, &clock_obj, &mut ctx);
...
fun claim_and_buy_rejects_second_claim_after_redeem() {
    ...
    let mut clock_obj = create_test_clock_at(test_scenario::ctx(&mut scn), 10);
...
fun claim_and_buy_item_with_discount_emits_events_and_covers_helpers() {
    ...
    let claimed_event_count_before = event::events_by_type<shop::DiscountClaimedEvent>().length();
    let purchase_event_count_before = event::events_by_type<shop::PurchaseCompletedEvent>().length();
    let redeemed_event_count_before = event::events_by_type<shop::DiscountRedeemedEvent>().length();

    let discount_id = shop::test_claim_and_buy_with_ids<TestItem, TestCoin>(
        &mut shared_shop,
        template_id,
        &price_info_obj,
        payment,
    ...
    assert_last_emitted!(
        claimed_event_count_before,
        shop::new_discount_claimed_event(shop_id, discount_id),
    );

    assert_last_emitted!(
        redeemed_event_count_before,
        shop::new_discount_redeemed_event(shop_id, template_id, discount_id),
    );
...
fun price_status_rejects_attestation_before_publish() {
    ...
    let mut clock_obj = create_test_clock_at(test_scenario::ctx(&mut scn), 1000);
...
fun setup_shop_with_listing_and_price_info(
    ...
    let (shop_id, _accepted_currency_id, listing_id, price_info_id) =
...
fun buy_item_emits_events_decrements_stock_and_refunds_change() {
    ...
    assert_eq!(
        stock_events[stock_events.length() - 1],
...
fun buy_item_rejects_out_of_stock_after_depletion() {
    ...
    let mut clock_obj = create_test_clock_at(test_scenario::ctx(&mut scn), 11);
    let payment = coin::mint_for_testing<TestCoin>(
        quote_amount,
        test_scenario::ctx(&mut scn),
    );
