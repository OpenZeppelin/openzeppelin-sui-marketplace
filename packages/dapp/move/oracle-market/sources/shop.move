...
fun claim_publisher(publisher_witness: SHOP, ctx: &mut tx::TxContext): pkg::Publisher {
    pkg::claim<SHOP>(publisher_witness, ctx)
}

fun transfer_publisher_to_sender(publisher: pkg::Publisher, ctx: &tx::TxContext) {
...
public entry fun disable_shop(shop: &mut Shop, owner_cap: &ShopOwnerCap, ctx: &mut tx::TxContext) {
...
public entry fun remove_accepted_currency(
    ...
    if (opt::is_some(&mapped_id_opt)) {
        assert!(*opt::borrow(&mapped_id_opt) == accepted_currency_id, EAcceptedCurrencyMissing);
    };
...
public entry fun attach_template_to_listing(
    ...
    item_listing.spotlight_discount_template_id = opt::some(template_id(discount_template));
}
...
fun add_currency_marker(shop: &mut Shop, accepted_currency_id: obj::ID, coin_type: TypeInfo) {
    dynamic_field::add(
        &mut shop.id,
        accepted_currency_id,
...
#[test_only]
public fun test_purchase_completed_coin_type(event: &PurchaseCompleted): TypeInfo {
    event.coin_type
}
...
#[test_only]
public fun test_minting_completed_coin_type(event: &MintingCompleted): TypeInfo {
    event.coin_type
}
