...
public(friend) fun add(parent_id: &mut UID, price_identifier: PriceIdentifier, id: ID) {
    assert!(!contains(parent_id, price_identifier), E_PRICE_IDENTIFIER_ALREADY_REGISTERED);
    table::add(
...
public fun get_id_bytes(parent_id: &UID, price_identifier: PriceIdentifier): vector<u8> {
    assert!(contains(parent_id, price_identifier), E_PRICE_IDENTIFIER_NOT_REGISTERED);
    object::id_to_bytes(
...
public fun get_balance(price_info_object: &PriceInfoObject): u64 {
    if (
        !dynamic_object_field::exists_with_type<vector<u8>, Coin<SUI>>(
            &price_info_object.id,
            FEE_STORAGE_KEY,
        )
    ) {
        return 0
    };
    let fee = dynamic_object_field::borrow<vector<u8>, Coin<SUI>>(
        &price_info_object.id,
        FEE_STORAGE_KEY,
    );
    coin::value(fee)
...
/// Dev-only entry point to mint a PriceInfoObject without an off-chain VAA.
/// Useful for localnet/mock environments; DO NOT use on shared networks.
public fun dev_new_price_info_object(
    price_info: PriceInfo,
    ctx: &mut TxContext,
): PriceInfoObject {
...
/// Convenience wrapper that builds a PriceFeed and PriceInfo in one call.
/// `price` and `ema_price` can be the same for simple mocks.
public fun dev_new_price_info_object_with_feed(
    ...
    let price_info = new_price_info(attestation_time, arrival_time, price_feed);
    new_price_info_object(price_info, ctx)
}
...
public fun test_get_price_info_object_id_from_price_identifier() {
    ...
    // Create a new ID.
    let id = object::id_from_bytes(
        x"19f253b07e88634bfd5a3a749f60bfdb83c9748910646803f06b60b76319e7ba",
    );
